  ...
  return (
      <div className={"flex flex-col gap-6 h-full w-full justify-center items-center"}>
        <h1>Datasets</h1>
          ...
          >
              <div
                  className="flex w-fit items-center justify-center gap-2 rounded-md border-2 border-dashed border-gray-300 px-4 py-2 hover:cursor-pointer">
          ...
          /> : datasets.map((dataset) => {
              return <div key={dataset.id}>
                      <p>{dataset.name}</p>
                      <a target="_blank" href={dataset.url}>Download</a>
                  </div>
              })}
              </div>
          </div>
